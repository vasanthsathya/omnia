...
def generate_inventory_for_node(node_info_db: tuple) -> None:
    ...
    Args:
        node_info_db (tuple): A tuple containing node information from the database,
            where hostname is at index 8, role string at index 9, and cluster name at index 10.
    Returns:
        None
    ...
    try:
        ...
        # Extract values from DB row
        hostname = node_info_db[8]
        role_str = node_info_db[9]
        cluster_name = node_info_db[10]

        # Skip if hostname or role is missing
        ...
        if cluster_name and cluster_name.strip():
            ...
            inventory_file = os.path.join(inventory_dir, "cluster_layout")
        ...
        # Prepare roles list, skipping empty/default ones
        roles = [r.strip() for r in role_str.split(",") if r.strip() and r.strip() != "default"]

        # Add hostname under each valid role
        for role in roles:
            # Clean service_ prefix except for service_node
            if role.startswith("service_") and role != "service_node":
                role = role[len("service_"):]
            if not config.has_section(role):
                config.add_section(role)
            if not config.has_option(role, hostname):
                config.set(role, hostname, None)

        # Save updated config
        with open(inventory_file, 'w', encoding='utf-8') as f:
            f.write(inventory_header)  # Write the header before the config
            config.write(f, space_around_delimiters=False)
        os.chmod(inventory_file, 0o444)  # Set file to read-only

    except Exception as e:
        syslog.syslog(syslog.LOG_ERR, f"generate_inventory_for_node: {type(e).__name__} {e}")
...
def generate_inventory_per_cluster(cursor) -> None:
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE hostname IS NOT NULL AND role IS NOT NULL AND cluster_name IS NOT NULL
        """)
        ...
        for cluster_name, role_map in cluster_map.items():
            ...
            for role, hostnames in role_map.items():
                if not config.has_section(role):
                    config.add_section(role)
