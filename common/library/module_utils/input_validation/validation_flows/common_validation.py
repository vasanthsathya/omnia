...
def validate_software_config(
    ...
    """
    Validates the software configuration for a cluster based on input data.

    Args:
        input_file_path (str): The path to the input file containing software configuration data.
        data (dict): A dictionary containing software configuration data.
        logger: A logger object used for logging.
        module: The name of the module being used.
        omnia_base_dir (str): The base directory of the Omnia infrastructure.
    ...
    if "additional_software" in data:
        ...
        schema_base_file_path = os.path.join(
            module_utils_base, 'input_validation', 'schema')
        passwords_set = config.passwords_set
        extensions = config.extensions
        fname = "additional_software"
        schema_file_path = schema_base_file_path + \
            "/" + fname + extensions['json']
        json_files = get.files_recursively(
            omnia_base_dir + "/" + project_name, extensions['json'])
        json_files_dic = {}

        for file_path in json_files:
            json_files_dic.update(
                {get.file_name_from_path(file_path): file_path})
        ...
        if False in vstatus:
            ...
            generate_log_failure_message(
                log_file_name, project_name, validation_status, module)

        # Check for the addtional_software.json file exist
        if new_file_path is None or not file_exists(
                new_file_path, module, logger):
            ...
            return errors
        additional_software_data = json.load(
            open(json_files_dic["additional_software.json"], "r"))
    ...
    # check if the corresponding json files for softwares and subgroups exists
    # in config folder
        ...
        if json_path is None:
            errors.append(
                create_error_msg(
                    "Validation Error: ",
                    software,
                    en_us_validation_msg.json_file_mandatory(json_path)))
        else:
            try:
                ...
                # eg: for amd the amd.json should contain both amd and rocm
                # entries
                ...
                errors.append(
                    create_error_msg(
                        "Error opening or reading JSON file:",
                        json_path,
                        str(e)))
...
def validate_security_config(
        ...
        project_name):
    """
    Validates the security configuration based on the input file path, data, logger, module
    omnia_base_dir, module_utils_base, and project_name.

    Parameters:
        input_file_path (str): The path to the input file.
        data (dict): The data to be validated.
        logger (Logger): The logger to be used.
        module (Module): The module to be used.
...
def validate_network_config(
    ...
    if mlnx_ofed_offline_path and not validation_utils.verify_path(
...
def validate_storage_config(
    ...
    if (contains_software(softwares, "slurm")
            and not slurm_share_val) or multiple_slurm_share_val:
        errors.append(
            create_error_msg(
                "slurm_share",
                slurm_share_val,
    ...
    if (contains_software(softwares, "k8s")
            and not k8s_share_val) or multiple_k8s_share_val:
        errors.append(
            create_error_msg(
                "k8s_share",
                k8s_share_val,
    ...
    if contains_software(softwares, "ucx") or contains_software(
            softwares, "openmpi"):
        ...
        if not beegfs_unmount_client:
            errors.append(
                create_error_msg(
                    ...
                    beegfs_unmount_client,
                    en_us_validation_msg.BEEGFS_UMOUNT_CLIENT_FAIL_MSG))
...
def validate_usernames(
    ...
    k8s_access_config_json = validation_utils.load_yaml_as_json(
        k8s_access_config_file_path, omnia_base_dir, project_name, logger, module)
    passwordless_ssh_config_file_path = create_file_path(
        input_file_path, file_names["passwordless_ssh_config"])
    passwordless_ssh_config_json = validation_utils.load_yaml_as_json(
        passwordless_ssh_config_file_path, omnia_base_dir, project_name, logger, module)
    ...
    duplicate_usernames = [username for username,
                           count in username_count.items() if count > 1]
...
def validate_server_spec(
    ...
    network_spec_file_path = create_file_path(
        input_file_path, file_names["network_spec"])
                ...
                if 'network' in item:
                    for network in item['network']:
                        for network_key, network_value in network.items():
                            ...
                            server_spec_nicnetworks.append(
                                network_value["nicnetwork"])
                            # Validating nicdevices if present
                            if 'nicdevices' in network_value:
                                if not network_key.startswith(
                                        network_value.get('nicdevices')):
                                    errors.append(
                                        create_error_msg(
                                            f'{network_key}',
                                            None,
...
def get_admin_bmc_networks(input_file_path, logger,
                           module, omnia_base_dir, project_name):
    """
    ...
    for k8s_cluster_type, k8s_clusters in cluster_set.items():
        ...
        for kluster in k8s_clusters:
            ...
            if deployment:
                ...
                csi_secret_path = kluster.get(
                    "csi_powerscale_driver_secret_file_path")
                csi_values_path = kluster.get(
                    "csi_powerscale_driver_values_file_path")

                # verify csi with sofwate config json
                if "csi_driver_powerscale" in softwares:
...
def validate_telemetry_config(
    ...
    federated_idrac_telemetry_collection = data.get(
...
def validate_additional_software(
    ...
    available_roles_and_groups = set(
        role['name'] for role in roles_config_json['Roles'])
    available_roles_and_groups.add("additional_software")

    # Add the set of all unique group names
    available_roles_and_groups.update(
        group for role in valid_roles for group in role['groups'])
    ...
    if "additional_software" not in software_config_json:
        logger.info(
            "The additional_software field is not present in software_config.json")
