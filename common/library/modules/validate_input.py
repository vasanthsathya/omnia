...
def main():
    ...
    json_files = fetch.files_recursively(
        omnia_base_dir + "/" + project_name, extensions['json'])
    yml_files = fetch.files_recursively(
        omnia_base_dir + "/" + project_name, extensions['yml'])
    schema_files = fetch.files_recursively(
        schema_base_file_path, extensions['json'])

    for file_path in json_files:
        json_files_dict.update({fetch.file_name_from_path(file_path): file_path})
    for file_path in yml_files:
        yml_files_dict.update({fetch.file_name_from_path(file_path): file_path})
    for file_path in schema_files:
        schema_files_dict.update(
            {fetch.file_name_from_path(file_path): file_path})
    ...
    if len(single_files) > 0:
        for name in single_files:
            ...
            logger.info("schema_file_path: %s", {schema_file_path})
            ...
            if name in json_files_dict:
                input_file_path = json_files_dict.get(name)
            if name in yml_files_dict:
                input_file_path = yml_files_dict.get(name)
    ...
    if len(tag_names) > 0 and ("all" not in tag_names or len(single_files) == 0):
        for tag_name in tag_names:
            if input_file_inventory.get(tag_name):
                for name in input_file_inventory[tag_name]:
                    ...
                    if not verify.file_exists(schema_file_path, module, logger):
                        error_message = f"The file schema: {fname}.json does not \
                         exist in directory: {schema_base_file_path}."
                        logger.info(error_message)
                        module.fail_json(msg=error_message)
                    ...
                    if input_file_path is None:
                        logger.error("file not found in directory: %s/%s",
                    ...
                    if schema_status:
                        logic_status = validate.logic(
                            input_file_path, logger, module, omnia_base_dir,
                            module_utils_base, project_name)

                    # Append the validation status for the input file
                    validation_status[project_name]["status"].append(
                        {input_file_path: "Passed"
    ...
    validation_status[project_name]["status"].sort(
...
def generate_log_failure_message(log_file_name, project_name, validation_status, module):
    ...
    module.fail_json(msg=message, log_file_name=log_file_name,
                     failed_files=failed_files, passed_files=passed_files)
