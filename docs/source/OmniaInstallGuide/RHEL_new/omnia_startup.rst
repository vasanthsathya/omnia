...
* Configures required mounts (*what mounts?*).
* Generates an SSH key pair and stores them in the ``/root/.ssh`` folder for both OIM and the containers.

.. caution::

   Do not delete any key pairs generated by Omnia from ``/root/.ssh``,
   as this will lead to Omnia execution failure.
...
* Creates the following directories within the OMNIA Core Container:
  ...
  /opt/workdir/omnia  
     Shared directory, mapped to ``$omnia_path`` in OIM  

  /opt/workdir/omnia/input  
     Playbook input files  

  omnia  
     OMNIA source code  

  /opt/workdir/omnia/log/core/playbooks  
