...
.. caution::
   ...
   because this will lead to ``omnia_core.service`` execution failure.

* Initializes the Podman container engine.
* Creates the following directories within the OMNIA Core Container:

  ``/opt/omnia``:  Shared directory that is mapped to the``$omnia_path`` in OIM.  
...
.. note::
    ...
    If other containers are present, run the following Ansible playbook to remove the containers::

    ansible-playbook oim_cleanup.yml

To uninstall the Omnia Core Container, do the following:
