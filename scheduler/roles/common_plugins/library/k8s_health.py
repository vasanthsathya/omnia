...
def load_k8s_config(config_path=None):
    ...
    try:
        ...
        try:
            ...
            raise ConfigException("Kubernetes configuration could not be loaded from any source.")
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            track_module_name=dict(type='str', required=False, default=None),
            track_file_path=dict(type='str', required=False, default=None),
            kubeconfig_path=dict(type='str', required=False, default=None)
        )
    )
